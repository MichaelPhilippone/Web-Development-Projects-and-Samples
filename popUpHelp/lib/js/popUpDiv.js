...
{	
	try {
		if (!document.getElementsByTagName("body")[0].parentElement) 
			window.browser = "ff";
		else 
			window.browser = "not ff";
		
		if (!propsObj.distFromTop) {
			propsObj.distFromTop = 100;
		}
		if (!propsObj.height) {
			propsObj.height = 500;
		}
		...
		if (propsObj.title) {
			window.titleBarText = propsObj.title
		}
		if (propsObj.redirectOnClose) {
			window.popUpCloseRedirect = propsObj.redirectOnClose;
		}
		if (propsObj.altCloseAction) {
			window.popUpCloseAction = propsObj.altCloseAction;
		}
...
{		
	...
	overlay.style.width = ((window.browser == "ff")?(window.innerWidth):('100%'));
	overlay.style.height = ((window.browser == "ff")?(window.innerHeight +  + getScrollXY()['y']):(document.getElementsByTagName("body")[0].offsetHeight + getScrollXY()['y']));
	overlay.style.display = 'block';
	overlay.style.position='absolute';
	overlay.style.top = '0'; //getScrollXY()['y'];
